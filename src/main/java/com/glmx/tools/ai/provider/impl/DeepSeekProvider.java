...
public class DeepSeekProvider extends AbstractModelProvider {
        ...
        try {
            ...
            try (Response response = httpClient.newCall(httpRequest).execute()) {
                if (!response.isSuccessful()) {
                    log.warn("Failed to fetch models from DeepSeek API: {}", response.code());
                    return Optional.empty();
                }

                String responseBody = response.body().string();
                List<ModelInfo> models = parseModelsResponse(responseBody);
                log.info("Fetched {} models from DeepSeek API", models.size());
                return Optional.of(models);
            }
        } catch (Exception e) {
            log.error("Error fetching models from DeepSeek API: {}", e.getMessage());
            return Optional.empty();
        }
    }
    ...
    private String formatModelDisplayName(String modelId) {
        if (modelId.contains("deepseek-chat")) return "DeepSeek Chat";
        if (modelId.contains("deepseek-coder")) return "DeepSeek Coder";
        if (modelId.contains("deepseek-v3")) return "DeepSeek V3";
        if (modelId.contains("deepseek-r1")) return "DeepSeek R1";
        return modelId;
    }

    private String getModelDescription(String modelId) {
        if (modelId.contains("chat")) return "对话模型";
        if (modelId.contains("coder")) return "代码生成模型";
        if (modelId.contains("v3")) return "最新版本";
        if (modelId.contains("r1")) return "推理模型";
        return "DeepSeek模型";
    }

    private boolean isVisionModel(String modelId) {
        return modelId.contains("vision");
    }

    @Override
    public ChatResponse chat(ChatRequest request) {
        log.info("Starting DeepSeek chat request with model: {}", request.getModel());
        ...
        try {
            ...
            try (Response response = httpClient.newCall(httpRequest).execute()) {
                ...
                if (!response.isSuccessful()) {
                    ...
                    throw new BusinessException("DeepSeek API错误 [" + response.code() + "]: " + parseErrorMessage(errorBody));
                }
            ...
            throw new BusinessException("DeepSeek API调用失败: " + e.getMessage());
        }
    }

    @Override
    public Flux<String> streamChat(ChatRequest request) {
        log.info("Starting DeepSeek stream chat request with model: {}", request.getModel());
        validateRequest(request);
    ...
    private String resolveApiKey(ChatRequest request) {
        ...
        if (defaultApiKey == null || defaultApiKey.isEmpty()) {
            throw new BusinessException("DeepSeek API Key未配置，请在设置中配置API Key");
        }
        return defaultApiKey;
    }
